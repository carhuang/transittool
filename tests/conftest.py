...
def sample_csv_path():
    """
    Return the first CSV file found in the tests/ directory (the sample Compass CSV).
    """
    tests_dir = Path(__file__).parent
    csv_files = list(tests_dir.glob("*.csv"))
    if not csv_files:
        raise FileNotFoundError("No CSV file found in tests/ — place your sample Compass CSV there")
    return csv_files[0]